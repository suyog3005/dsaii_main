...
export function MovieGrid() {
  const gridData = useMemo(() => {
    ...
    for (let i = 0; i < COLS * ROWS; i++) {
      ...
      data.push({ title, genre: genre as keyof typeof G, idx: i });
    }
    return data;
  }, []);
      ...
      {Array.from({ length: COLS }, (_, colIdx) => (
        ...
        >
          {Array.from({ length: ROWS }, (_, rowIdx) => {
            ...
            return (
              <div key={cellIdx} style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <LegendCell title={item.title} genre={item.genre} idx={cellIdx} />
              </div>
            );
          })}
        </motion.div>
